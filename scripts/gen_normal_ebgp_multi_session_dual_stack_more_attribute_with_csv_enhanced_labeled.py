...
# ------------ Scenario 10: IPv4/IPv6 Withdrawals Using MP_UNREACH_NLRI ------------
...
# ------------ Scenario 11: BGP NOTIFICATION Messages ------------
...
def extract_bgp_to_csv(packets, csv_file):
    ...
    attack_prefixes = set()  # Will contain prefixes from attack scenarios
        ...
        for pkt_index, pkt in enumerate(packets, 1):  # Start from 1 to match tshark
            if BGPHeader in pkt and pkt[BGPHeader].type == 2:  # BGP UPDATE
                ...
                for prefix in nlri_prefixes:
                    announce_count += 1
                   # Default label is "normal"
                    label = "normal"
                    # Check if this is an attack prefix
                    if prefix in attack_prefixes:
                        label = "abnormal"
                    ...
                    writer.writerow([
                        ...
                        prefix, as_path, origin, next_hop, med,
                        local_pref, communities, aggregator_flag, aggregator_info, label
                    ])
                ...
                for prefix in withdrawn_prefixes:
                    withdraw_count += 1
                    label = "abnormal" if prefix in attack_prefixes else "normal"
                    # Update counters
                    if label == "normal":
                        normal_count += 1
                    else:
    ...
    print(f"  Normal traffic: {normal_count} packets")
    print(f"  Abnormal traffic: {abnormal_count} packets")
    print(f"  Total announcements: {announce_count}")
    print(f"  Total withdrawals: {withdraw_count}")

    return announce_count, withdraw_count, normal_count, abnormal_count
...
announce_count, withdraw_count, normal_count, abnormal_count = extract_bgp_to_csv(pkts, csv_filename)
