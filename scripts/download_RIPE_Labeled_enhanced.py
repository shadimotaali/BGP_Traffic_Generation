...
CSV_OUTPUT = os.path.join(RIPE_DIR, "rrc04_20251116_updates.csv")
TEMP_DIR = os.path.join(RIPE_DIR, "temp_mrt")

# Time range - November 16, 2025 00:05 to November 17, 2025 00:00
START_FILE = "updates.20251116.0005.gz"
END_FILE = "updates.20251117.0000.gz"
...
def parse_bgpdump_line(line):
    ...
    try:
        ...
        # Extract origin AS from AS_PATH
        origin_as = ''
        if as_path:
...
def collect_and_process_updates():
    ...
    start_time = datetime.strptime("20251116.0005", "%Y%m%d.%H%M")
    end_time = datetime.strptime("20251117.0000", "%Y%m%d.%H%M")
    ...
    if all_records:
        ...
        try:
            ...
            with_origin = sum(1 for r in all_records if r.get('Origin_AS'))
