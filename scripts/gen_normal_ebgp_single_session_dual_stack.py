from scapy.all import IP, IPv6, TCP, Ether, Padding, wrpcap
...
# NEXT_HOP attribute for direct IPv4 advertisement
next_hop_attr_v4 = BGPPathAttr(type_flags=0x40, type_code=3)
next_hop_attr_v4.attribute = BGPPANextHop(next_hop=ipv4_next_hop)

# Combine path attributes for direct IPv4 advertisement
ipv4_update_v4.path_attr = [origin, as_path_attr, next_hop_attr_v4, med_attr]
...
# 3. MP_REACH_NLRI attribute for IPv6
...
# Combine path attributes for IPv6 over IPv4 update
ipv6_update_v4.path_attr = [origin, as_path_attr, mp_reach_attr_v4, med_attr]
...
# Combine path attributes for IPv6 update over IPv6 session
ipv6_update_v6.path_attr = [origin, as_path_attr, mp_reach_attr_v6, med_attr]
...
# Combine path attributes for IPv4 update over IPv6 session
ipv4_update_v6.path_attr = [origin, as_path_attr, mp_reach_ipv4_attr, med_attr]
...
# Write to pcap
wrpcap("pcaps/full_dual_stack_bgp.pcap", pkts)
print("✅ Wrote pcaps/full_dual_stack_bgp.pcap with both IPv4 and IPv6 BGP sessions")