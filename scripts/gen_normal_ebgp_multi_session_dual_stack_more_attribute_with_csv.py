...
try:
    ...
    print("[*] Falling back to a standard IPv4 withdrawal")
    
    # We'll create a withdrawal for one of the IPv4 prefixes we previously announced
    # This maintains the scenario's intent (withdrawing a route) without dummy data
    fallback_prefix = ipv4_nlri[0]  # Use an existing prefix from your topology
    mp_unreach_withdraw_v4 = BGPHeader(type=2)/BGPUpdate()
    
    # Create a proper withdrawal NLRI
    fallback_nlri = BGPNLRI_IPv4(prefix=fallback_prefix)
    mp_unreach_withdraw_v4.withdrawn = [fallback_nlri]
    
    # Set withdraw_len correctly for Scapy versions that need it
    if hasattr(mp_unreach_withdraw_v4, 'withdraw_len'):
        mp_unreach_withdraw_v4.withdraw_len = len(raw(fallback_nlri))
    
    # Skip the MP_UNREACH_NLRI attribute since we're using standard withdrawal
    mp_unreach_attr_v4 = None
    ...
    with open(csv_file, 'w', newline='') as f:
        ...
        for pkt_index, pkt in enumerate(packets, 1):  # Start from 1 to match tshark
            if BGPHeader in pkt and pkt[BGPHeader].type == 2:  # BGP UPDATE
                ...
                if hasattr(bgp_update, 'withdrawn') and bgp_update.withdrawn:
                    for w in bgp_update.withdrawn:
                        if hasattr(w, 'prefix'):
                ...
                if hasattr(bgp_update, 'path_attr') and bgp_update.path_attr:
                    for attr in bgp_update.path_attr:
                        ...
                        # MP_UNREACH_NLRI (type 15)
                            ...
                            has_mp_unreach = True
                            # First try standard attribute fields
                                    ...
                                    if isinstance(field_value, list):
                                        for item in field_value:
                                            if hasattr(item, 'prefix'):
                                                mp_withdrawn_prefixes.append(item.prefix)
                                                prefixes_found = True
                            
                            
                            # If no prefixes found via standard attributes, try direct binary parsing
                            if not prefixes_found:
                               if hasattr(attr.attribute, 'afi') and hasattr(attr.attribute, 'safi'):
                                   afi = attr.attribute.afi
                                   safi = attr.attribute.safi
                                   mp_withdrawn_prefixes.append(f"MP_UNREACH_NLRI:AFI{afi}/SAFI{safi}")
                ...
                if (not nlri_prefixes) and (not mp_reach_prefixes) and \
                   (not withdrawn_prefixes) and (not mp_withdrawn_prefixes) and \
                   (not hasattr(bgp_update, 'path_attr') or not bgp_update.path_attr):
                   # This is an empty UPDATE which is a withdrawal message
                   # In true BGP protocol, this would withdraw routes based on session state
                   # We'll create a special withdrawal entry to indicate this occurred
                   withdraw_count += 1
                   writer.writerow([
                ...
                # Make sure MP_UNREACH packets are marked as withdrawals even if no specific prefixes were found
